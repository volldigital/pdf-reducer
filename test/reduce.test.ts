...
// sharp's Create type wrongly requires `background`; `noise` fills the canvas at
// runtime, so cast rather than add one (which changes the generated bytes).
const bigNoisyJpg = (w = 3000, h = 2250): Promise<Buffer> =>
  sharp({
    create: {
      width: w,
      height: h,
...
const bigNoisyGrayJpg = (w = 3000, h = 2250): Promise<Buffer> =>
  sharp({
    create: {
      width: w,
      height: h,
...
async function pdfWithImage(jpgBuffer: Buffer): Promise<string> {
