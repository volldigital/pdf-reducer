...
async function pdfWithImages(count) {
...
async function firstImageWidth(base64) {
...
test('non-object options are ignored (defaults used)', async () => {
  const input = await pdfWithImages(1);
  assert.equal(await reduce(input, null), await reduce(input));
  assert.equal(await reduce(input, 'nonsense'), await reduce(input));
});
