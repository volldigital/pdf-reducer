...
### D26. Publish as the org-scoped package `@disphere/pdf-reducer` (supersedes the unscoped-name plan in D21/D25)
**Decision:** Publish under the `disphere` npm organization as the scoped package `@disphere/pdf-reducer`, not the unscoped `pdf-reducer`.
**Why:** The package must be owned by the `disphere` org. Publishing a package whose scope matches the org name makes the org the owner **automatically on first publish** ([npm: About organization scopes and packages](https://docs.npmjs.com/about-organization-scopes-and-packages/)), whereas an unscoped package's first publish is owned by the publishing *user* and must then be transferred to the org (via Org → Teams → Packages → "Add Existing Package", or `npm access grant`). An initial unscoped bootstrap publish was made under a personal account and then unpublished; npm's [unpublish policy](https://docs.npmjs.com/policies/unpublish) then (a) burns the exact `pdf-reducer@0.1.0` version permanently and (b) locks the unscoped `pdf-reducer` name for 24 hours — friction the scoped name avoids entirely, since `@disphere/pdf-reducer` is a fresh, never-published identity (version `0.1.0` is reusable, no name lock). Consumer install becomes `npm install @disphere/pdf-reducer`; `publishConfig.access: public` keeps the scoped package public (scoped packages are private by default). CLI command names stay `pdf-reducer`/`pdf-reducer-analyze` (bin names need not be scoped). Trusted Publishing (D23) is unaffected — it is configured per package regardless of scope.

### D27. Node.js support policy `>=22`; CI matrix `[22, 24, 26]`; actions pinned to current majors
**Decision:** Set `engines.node` to `>=22`, test CI on Node 22 (Maintenance LTS), 24 (Active LTS), and 26 (Current), run the release job on Node 24, and pin the workflow actions to their current majors (`actions/checkout@v7`, `actions/setup-node@v6`).
**Why:** Node 18 and 20 are end-of-life (EOL Apr 2025 and ~Apr 2026 per nodejs.org), so the old `[18, 20, 22]` matrix exercised dead runtimes; testing only supported lines keeps the compatibility promise honest. A separate issue was the "Node.js 20 actions are deprecated" CI warning — that came from the GitHub Actions *runtime* used by `checkout@v4`/`setup-node@v4` (a Node 20 runtime GitHub removes from runners on 2026-09-16), **independent of the test matrix**, which is why all matrix legs emitted it; upgrading to `checkout@v7`/`setup-node@v6` (Node 24 runtime) removes it. The release job moved off EOL Node 20 to Active LTS Node 24; the explicit `npm install -g npm@11` step is retained because Node 24's bundled npm can be below the Trusted-Publishing minimum (`11.5.1`).
...
## 2026-07-14 — Fix CommonJS `require()` resolution

### D32. Use a `default` export condition instead of `import`; raise `engines` to `>=22.12` (amends D30)
**Decision:** Change the `exports["."]` condition from `import` to `default` (still pointing at the single ESM artifact `./dist/pdfSizeReducer.js`), and bump `engines.node` from `>=22` to `>=22.12`. The package remains ESM-only — no CJS build was added, so D30's core decision stands.
**Why:** A plain-JavaScript CommonJS consumer loading the library with `const { reduce } = require("@disphere/pdf-reducer")` failed with `ERR_PACKAGE_PATH_NOT_EXPORTED` — `No "exports" main defined`. The `exports` map offered only the `import` condition, so when Node resolved the `.` subpath under the `require` condition set, **no condition matched** and it reported no resolvable main. (The `bin` CLIs were unaffected because they reference `dist/bin/*.js` by path, bypassing `exports` entirely.) D30's assumption — that CJS consumers "always had to use dynamic `import()`" — was wrong for Node ≥22.12, which can `require()` an ESM module directly (unflagged `require(esm)`); the only thing missing was a condition the `require` resolver would use. `default` matches every resolver (ESM `import`, CJS `require`, bundlers) and targets the same ESM file, so both `import` and `require` now resolve without a second build or the dual-package hazard. This is safe here because the module graph has **no top-level await** (every `await` is inside an `async` function; deps `sharp`/`pdf-lib` are CJS), so `require(esm)` won't throw `ERR_REQUIRE_ASYNC_MODULE`. Because `require(esm)` is only unflagged from Node 22.12.0, `engines.node` was tightened to `>=22.12` to make the CJS floor explicit (a CJS consumer on 22.0–22.11 would otherwise hit `ERR_REQUIRE_ESM`). Requires a patch republish so consumers install the corrected `package.json`.
